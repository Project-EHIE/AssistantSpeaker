...
def adjust_hour_based_on_context(hour, context):
    pm_indicators = {'pm', 'afternoon', 'evening', 'night'}
    context_words = set(context.lower().split())

    if context_words & pm_indicators:
        if hour == 12:
            adjusted_hour = 12
        elif hour < 12:
...
def parse_minute_and_adjust_hour(full_match, position, hour):
    ...
    if position == 'past':
        ...
        minute = 30 if 'half' in full_match else 0
    return minute, hour
...
def get_alarms():
    ...
    c.execute("SELECT alarm_time FROM alarms")
    rows = c.fetchall()
    if rows:
